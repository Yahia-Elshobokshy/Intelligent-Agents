...
if not os.path.exists(ENCODINGS_PATH):
    raise FileNotFoundError(
        "Missing encodings.pkl at: %s\n"
        "Run: python \"%s\" first (it generates encodings.pkl)."
        % (ENCODINGS_PATH, os.path.join(SCRIPT_DIR, "encode_faces.py"))
    )

with open(ENCODINGS_PATH, "rb") as f:
    known_encodings, known_names = pickle.load(f)
print("[BOOT] %d faces loaded for: %s" % (len(known_encodings), sorted(set(known_names))))
...
def process_loop():
    ...
    print("[RUN] Mode: %s  -- press ESC to quit" % MODE)
    ...
    while True:
        ...
        for (t, r, b, l), enc in zip(locations, encodings):
            ...
            if frame_count % SPOOF_INTERVAL == 0 or fid not in liveness_cache:
                is_live, conf, _ = check_liveness(frame)  # use repo's detector
            ...
            # -- Color & status ----------------------------------------------
            if grant:
                # Known + Live = GREEN (access granted)
                color = (0, 255, 0)
                status = "ACCESS"
            elif name != "Unknown" and not is_live:
                # Known + Spoof = ORANGE (spoof attack on known person)
                color = (0, 140, 255)
                status = "SPOOF"
            elif is_live:
                # Live but Unknown = YELLOW (live stranger)
                color = (0, 255, 255)
                status = "LIVE"
            else:
                # Spoof and Unknown = RED
                color = (0, 0, 255)
                status = "SPOOF"

            conf_pct = int(conf * 100)
            label    = "%s  %s %d%%" % (name, status, conf_pct)
            ...
            # Confidence bar (always shows liveness confidence)
            bar_w  = r - l
            filled = int(bar_w * np.clip(conf, 0, 1))
            bar_c  = (0, 255, 0) if is_live else (0, 0, 255)
            cv2.rectangle(frame, (l, b + 4), (r, b + 12), (50, 50, 50), -1)
            cv2.rectangle(frame, (l, b + 4), (l + filled, b + 12), bar_c, -1)

            # Keep best result for ESP32 decision endpoint
        ...
        cv2.putText(frame, "[%s]  ESC = quit" % mode_label,
...
if __name__ == "__main__":
    if MODE == "esp32":
        ...
        flask_thread.start()
        print("[BOOT] Flask listening on port %d" % FLASK_PORT)
